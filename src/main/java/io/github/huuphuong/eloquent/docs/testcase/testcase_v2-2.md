...
|---|---|---:|---|
| User cÃ³ trong relation nhÆ°ng khÃ´ng cÃ³ profile row | `EloquentAdvancedCoverageTest` | Covered | `with("users.profile")` váº«n attach `null` cho profile thiáº¿u |

### 1b. `hasOne` - duplicate rows / order handling

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| Má»™t user cÃ³ nhiá»u profile row | `EloquentAdvancedCoverageTest` | Covered | `with("users.profile", q -> q.orderBy("id", "DESC"))` chá»n profile cÃ³ id lá»›n hÆ¡n |

### 2. `hasOneThroughPivot` - null path

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| Department khÃ´ng cÃ³ pivot row `MANAGER` | `EloquentAdvancedCoverageTest` | Covered | `with("manager")` attach `null` Ä‘Ãºng nghÄ©a |

### 2b. `hasOneThroughPivot` - duplicate rows / order handling

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| Department cÃ³ nhiá»u pivot row `MANAGER` | `EloquentAdvancedCoverageTest` | Covered | `with("manager", q -> q.orderBy("id", "DESC"))` chá»n user id lá»›n hÆ¡n |

### 3. Nested `with(...)` callback error

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| Path sÃ¢u hÆ¡n nhÆ°ng relation con khÃ´ng tá»“n táº¡i | `EloquentAdvancedCoverageTest` | Covered | `with("users", q -> q.with("profile.ghost"))` fail rÃµ rÃ ng |

### 4. Offset pagination trÃªn dataset lá»›n hÆ¡n

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| 30 departments, page size 10 | `EloquentAdvancedCoverageTest` | Covered | Check page 1/2/3/4, total, totalPages, hasNext/hasPrevious |

### 5. Cursor pagination + eager loading

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| `cursorPaginate()` vá»›i `with("users.profile")` | `EloquentAdvancedCoverageTest` | Covered | Cursor paging váº«n attach graph á»Ÿ page Ä‘áº§u |
| `cursorPaginate()` vá»›i `with("users.profile")` + `with("manager")` | `EloquentAdvancedCoverageTest` | Covered | Page sau váº«n cÃ³ nested graph; page 2 cÃ³ Ã­t nháº¥t má»™t item cÃ³ manager |

### 6. `whereJson(...)` invalid object

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| Object khÃ´ng serialize Ä‘Æ°á»£c | `EloquentAdvancedCoverageTest` | Covered | Bá»c thÃ nh `IllegalArgumentException` |
| Map tá»± tham chiáº¿u | `EloquentAdvancedCoverageTest` | Covered | Cycle cÅ©ng fail fast |

### 7. `whereDate(...)` vá»›i kiá»ƒu khÃ¡c `LocalDate`

| Case | Test file | Tráº¡ng thÃ¡i | Ghi chÃº |
|---|---|---:|---|
| `LocalDateTime` | `EloquentAdvancedCoverageTest` | Covered | Normalize vá» ngÃ y |
| `ZonedDateTime` | `EloquentAdvancedCoverageTest` | Covered | Normalize vá» ngÃ y |
